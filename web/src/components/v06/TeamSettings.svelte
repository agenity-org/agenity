...
<div class="backdrop" on:click={onClose}>
  <div class="modal" on:click|stopPropagation>
    ...
    <div class="body">
      {#if tab === 'members'}
        <p class="hint">All agents in this team. Right-click on the workspace session-list to move members between teams or change roles.</p>
        <table>
          <thead><tr><th>Name</th><th>Role</th><th>Agent</th><th>Joined</th></tr></thead>
          <tbody>
            {#each (members || []) as m}
              <tr>
                <td>{m.agent_name}</td>
            ...
            {/each}
            {#if !members?.length}<tr><td colspan="4" class="empty">No memberships yet.</td></tr>{/if}
          </tbody>
        </table>
      {:else if tab === 'canon'}
