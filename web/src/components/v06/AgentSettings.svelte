...
<script>
  ...
  // Membership state
  let changingTeam = $state(false);
  let pickTeam = $state(agent?.team || '');
  let pickRole = $state(agent?.role || 'worker');
  ...
  <div class="modal" on:click|stopPropagation>
    ...
    <div class="body">
      ...
      {:else if tab === 'membership'}
        ...
        <div class="row"><button class="primary" on:click={changeTeam} disabled={changingTeam || (pickTeam === agent?.team && pickRole === agent?.role)}>{changingTeam ? 'Moving…' : 'Apply'}</button></div>
      {:else if tab === 'actions'}
        <p class="hint">Direct PTY / lifecycle actions. Restart preserves cwd + role + prompt + stat sheet; Stop tears down.</p>
        <div class="actions">
