...
func renderAgentClaudeMD(spec SpawnSpec, peers []PeerBrief) string {
	...
	fmt.Fprintf(&b, "DO NOT write `@<peer>: <message>` in your own output thinking it'll be routed — outbound MCP messages must use the `chepherd.send_to_session` tool call. Only INBOUND knock markers from peers surface in your pane.\n\n")

	// #451 — V0.9.2-ARCHITECTURE §10 Pattern 1 knock-fetch instructions.
	fmt.Fprintf(&b, "### Inbound peer messages — the knock pattern\n\n")
	fmt.Fprintf(&b, "When a peer sends you a message via `chepherd.send_to_session`, chepherd writes a **knock marker** to your terminal — the marker is a notification, NOT the message body. It looks like:\n\n")
	fmt.Fprintf(&b, "```\n[chepherd-knock taskID=<uuid> from=<peer-name>]\n```\n\n")
	fmt.Fprintf(&b, "When you see this line:\n\n")
	fmt.Fprintf(&b, "1. Extract the `taskID=<uuid>` value\n")
	fmt.Fprintf(&b, "2. Call `chepherd.get_task(taskID=<that-uuid>)` to pull the structured A2A task envelope — that's where the actual message body lives.\n")
	fmt.Fprintf(&b, "3. The response carries `{taskID, state, from, contextID, body, createdAt}`. The `body` field is the peer's message text.\n")
	fmt.Fprintf(&b, "4. Process the message + reply via normal stdout — chepherd's silence-detect captures your response + completes the task back to the sender.\n\n")
	fmt.Fprintf(&b, "**Do NOT treat the knock line itself as content.** It's just the doorbell; the actual letter is behind `get_task`. This is the V0.9.2-ARCHITECTURE §10 Pattern 1 contract: runner-to-PTY carries notifications only; structured payloads flow via MCP.\n\n")
	fmt.Fprintf(&b, "**Use `chepherd.list_peers()` to discover who's in your team.** It returns Agent Cards (name + role + skills) for team peers, scoped to YOUR team — distinct from `chepherd.list_sessions` which returns everyone globally.\n\n")
	...
	fmt.Fprintf(&b, "- `chepherd.list_peers` — enumerate YOUR team's peers (Agent Cards). Team-scoped; distinct from chepherd.list_sessions (#451)\n")
	fmt.Fprintf(&b, "- `chepherd.list_sessions` — enumerate ALL sessions globally\n")
	...
	fmt.Fprintf(&b, "- `chepherd.send_to_session` — message a peer (recipient gets a knock; calls get_task to pull body)\n")
	fmt.Fprintf(&b, "- `chepherd.get_task` — pull the A2A task envelope by taskID. **Call this when you see a `[chepherd-knock ...]` line** to read the actual peer message (#451)\n")
