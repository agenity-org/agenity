...
func (r *AgentRepository) Get(ctx context.Context, id string) (*persistence.Agent, error) {
	if id == "" {
		return nil, errors.New("postgres AgentRepository: empty id")
	}
	var (
		a             persistence.Agent
		skillsJSON    string
	...
	if errors.Is(err, sql.ErrNoRows) {
		return nil, fmt.Errorf("postgres agents Get %q: not found", id)
	}
	if err != nil {
		return nil, fmt.Errorf("postgres agents Get %q: %w", id, err)
	}
...
func (r *AgentRepository) List(ctx context.Context) ([]*persistence.Agent, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres agents List: %w", err)
	}
	defer rows.Close()
	var ids []string
	for rows.Next() {
...
func (r *AgentRepository) Save(ctx context.Context, a *persistence.Agent) error {
	if a == nil {
		return errors.New("postgres AgentRepository: nil agent")
	}
	if a.ID == "" {
		return errors.New("postgres AgentRepository: empty agent ID")
	}
	if a.Type == "" {
		return errors.New("postgres AgentRepository: empty agent Type")
	}
	...
	_, err = r.db.ExecContext(ctx,
		...
		pgJSONOrEmpty(a.Metadata), a.CreatedAt, a.UpdatedAt,
	)
	if err != nil {
		return fmt.Errorf("postgres agents Save %q: %w", a.ID, err)
	}
	return nil
}
...
func (r *AgentRepository) Delete(ctx context.Context, id string) error {
	if id == "" {
		return errors.New("postgres AgentRepository: empty id")
	}
	_, err := r.db.ExecContext(ctx,
		`DELETE FROM agents WHERE id = $1`, id,
	)
	if err != nil {
		return fmt.Errorf("postgres agents Delete %q: %w", id, err)
	}
	return nil
}
