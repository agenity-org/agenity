...
func (r *PushNotificationConfigRepository) Get(ctx context.Context, id string) (*persistence.PushNotificationConfig, error) {
	if id == "" {
		return nil, errors.New("postgres PushNotificationConfigRepository: empty id")
	}
	var (
	...
	if errors.Is(err, sql.ErrNoRows) {
		return nil, fmt.Errorf("postgres push_notification_configs Get %q: not found", id)
	}
	if err != nil {
		return nil, fmt.Errorf("postgres push_notification_configs Get %q: %w", id, err)
	}
	if filtersJSON != "" {
...
func (r *PushNotificationConfigRepository) List(ctx context.Context, taskID string) ([]*persistence.PushNotificationConfig, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres push_notification_configs List: %w", err)
	}
	defer rows.Close()
	var ids []string
	for rows.Next() {
...
func (r *PushNotificationConfigRepository) Save(ctx context.Context, c *persistence.PushNotificationConfig) error {
	if c == nil {
		return errors.New("postgres PushNotificationConfigRepository: nil config")
	}
	if c.ID == "" {
		return errors.New("postgres PushNotificationConfigRepository: empty ID")
	}
	if c.TaskID == "" {
		return errors.New("postgres PushNotificationConfigRepository: empty TaskID")
	}
	if c.URL == "" {
		return errors.New("postgres PushNotificationConfigRepository: empty URL")
	}
	if c.Filters == nil {
		c.Filters = []string{}
	}
	...
	if err != nil {
		return fmt.Errorf("postgres push_notification_configs Save %q: %w", c.ID, err)
	}
	return nil
}
...
func (r *PushNotificationConfigRepository) Delete(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("postgres push_notification_configs Delete %q: %w", id, err)
	}
	return nil
}
