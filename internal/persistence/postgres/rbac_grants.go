...
func (r *RBACGrantRepository) Get(ctx context.Context, id string) (*persistence.Grant, error) {
	if id == "" {
		return nil, errors.New("postgres RBACGrantRepository: empty id")
	}
	var (
		g            persistence.Grant
		scopeJSON    string
	...
	if errors.Is(err, sql.ErrNoRows) {
		return nil, fmt.Errorf("postgres rbac_grants Get %q: not found", id)
	}
	if err != nil {
		return nil, fmt.Errorf("postgres rbac_grants Get %q: %w", id, err)
	}
	if err := json.Unmarshal([]byte(scopeJSON), &g.Scope); err != nil {
		return nil, fmt.Errorf("postgres rbac_grants Get %q scope: %w", id, err)
	}
	if err := json.Unmarshal([]byte(permsJSON), &g.Permissions); err != nil {
		return nil, fmt.Errorf("postgres rbac_grants Get %q perms: %w", id, err)
	}
	if rateJSON != "null" && rateJSON != "" {
		var rl persistence.GrantRateLimit
		if err := json.Unmarshal([]byte(rateJSON), &rl); err != nil {
			return nil, fmt.Errorf("postgres rbac_grants Get %q rate: %w", id, err)
		}
		g.RateLimit = &rl
	}
	if expiresAt.Valid {
...
func (r *RBACGrantRepository) List(ctx context.Context, opts persistence.GrantListOpts) ([]*persistence.Grant, error) {
	...
	if opts.OnlyActive {
		conds = append(conds, "accepted = TRUE")
	...
	if err != nil {
		return nil, fmt.Errorf("postgres rbac_grants List: %w", err)
	}
	defer rows.Close()
	var ids []string
	for rows.Next() {
...
func (r *RBACGrantRepository) Save(ctx context.Context, g *persistence.Grant) error {
	if g == nil {
		return errors.New("postgres RBACGrantRepository: nil grant")
	}
	if g.ID == "" {
		return errors.New("postgres RBACGrantRepository: empty grant ID")
	}
	if g.GranterOrg == "" || g.GranteeOrg == "" {
		return errors.New("postgres RBACGrantRepository: empty granter/grantee org")
	}
	...
	_, err = r.db.ExecContext(ctx,
		...
		rate, expires, g.Accepted, g.CreatedBy, g.CreatedAt,
	)
	if err != nil {
		return fmt.Errorf("postgres rbac_grants Save %q: %w", g.ID, err)
	}
	return nil
}
...
func (r *RBACGrantRepository) Delete(ctx context.Context, id string) error {
	if id == "" {
		return errors.New("postgres RBACGrantRepository: empty id")
	}
	_, err := r.db.ExecContext(ctx,
		`DELETE FROM rbac_grants WHERE id = $1`, id,
	)
	if err != nil {
		return fmt.Errorf("postgres rbac_grants Delete %q: %w", id, err)
	}
	return nil
}
