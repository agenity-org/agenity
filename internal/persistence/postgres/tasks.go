...
func (r *TaskRepository) Get(ctx context.Context, taskID string) (*persistence.Task, error) {
	if taskID == "" {
		return nil, errors.New("postgres TaskRepository: empty taskID")
	}
	...
	if errors.Is(err, sql.ErrNoRows) {
		return nil, fmt.Errorf("postgres tasks Get %q: not found", taskID)
	}
	if err != nil {
		return nil, fmt.Errorf("postgres tasks Get %q: %w", taskID, err)
	}
	return &t, nil
}
...
func (r *TaskRepository) Save(ctx context.Context, t *persistence.Task) error {
	if t == nil {
		return errors.New("postgres TaskRepository: nil task")
	}
	if t.ID == "" {
		return errors.New("postgres TaskRepository: empty task ID")
	}
	if t.RunnerSID == "" {
		return errors.New("postgres TaskRepository: empty RunnerSID")
	}
	if t.State == "" {
		return errors.New("postgres TaskRepository: empty State")
	}
	now := time.Now().UTC()
	...
	if err != nil {
		return fmt.Errorf("postgres tasks Save %q: %w", t.ID, err)
	}
	return nil
}
...
func (r *TaskRepository) List(ctx context.Context, opts persistence.TaskListOpts) ([]*persistence.Task, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres tasks List: %w", err)
	}
	defer rows.Close()
...
func (r *TaskRepository) Delete(ctx context.Context, taskID string) error {
	if taskID == "" {
		return errors.New("postgres TaskRepository: empty taskID")
	}
	_, err := r.db.ExecContext(ctx,
		`DELETE FROM tasks WHERE id = $1`, taskID,
	)
	if err != nil {
		return fmt.Errorf("postgres tasks Delete %q: %w", taskID, err)
	}
	return nil
}
