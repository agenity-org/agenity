...
func (r *AgentCardRepository) Get(ctx context.Context, agentSID string) (*persistence.AgentCard, error) {
	if agentSID == "" {
		return nil, errors.New("postgres AgentCardRepository: empty SID")
	}
	var (
	...
	if errors.Is(err, sql.ErrNoRows) {
		return nil, fmt.Errorf("postgres agent_cards Get %q: not found", agentSID)
	}
	if err != nil {
		return nil, fmt.Errorf("postgres agent_cards Get %q: %w", agentSID, err)
	}
...
func (r *AgentCardRepository) Save(ctx context.Context, c *persistence.AgentCard) error {
	if c == nil {
		return errors.New("postgres AgentCardRepository: nil card")
	}
	if c.SID == "" {
		return errors.New("postgres AgentCardRepository: empty SID")
	}
	if c.Name == "" {
		return errors.New("postgres AgentCardRepository: empty Name")
	}
	if len(c.Body) == 0 {
		return errors.New("postgres AgentCardRepository: empty Body")
	}
	...
	_, err := r.db.ExecContext(ctx,
		...
		c.SID, c.Name, c.Body, c.PublicVisibility,
		syncedAt, c.UpdatedAt,
	)
	if err != nil {
		return fmt.Errorf("postgres agent_cards Save %q: %w", c.SID, err)
	}
	return nil
}
...
func (r *AgentCardRepository) List(ctx context.Context, opts persistence.AgentCardListOpts) ([]*persistence.AgentCard, error) {
	...
	idx := 1
	if opts.PublicOnly {
		conds = append(conds, "public_visibility = TRUE")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("postgres agent_cards List: %w", err)
	}
	defer rows.Close()
	var sids []string
	for rows.Next() {
...
func (r *AgentCardRepository) Delete(ctx context.Context, agentSID string) error {
	...
	if err != nil {
		return fmt.Errorf("postgres agent_cards Delete %q: %w", agentSID, err)
	}
	return nil
}
