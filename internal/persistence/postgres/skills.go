...
func (r *SkillRepository) Get(ctx context.Context, id string) (*persistence.Skill, error) {
	if id == "" {
		return nil, errors.New("postgres SkillRepository: empty id")
	}
	var (
		s        persistence.Skill
		readOnly bool
	...
	if errors.Is(err, sql.ErrNoRows) {
		return nil, fmt.Errorf("postgres skills Get %q: not found", id)
	}
	if err != nil {
		return nil, fmt.Errorf("postgres skills Get %q: %w", id, err)
	}
	s.ReadOnly = readOnly
	return &s, nil
}
...
func (r *SkillRepository) List(ctx context.Context, opts persistence.SkillListOpts) ([]persistence.Skill, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres skills List: %w", err)
	}
	defer rows.Close()
...
func (r *SkillRepository) Save(ctx context.Context, s *persistence.Skill) error {
	if s == nil {
		return errors.New("postgres SkillRepository: nil skill")
	}
	if s.ID == "" {
		return errors.New("postgres SkillRepository: empty skill ID")
	}
	...
	_, err := r.db.ExecContext(ctx,
		...
		s.ID, s.Name, s.DefaultBody, s.OverrideBody, s.ReadOnly,
		s.Source, s.Path, s.SortOrder, pgJSONOrEmpty(s.Metadata), s.UpdatedAt,
	)
	if err != nil {
		return fmt.Errorf("postgres skills Save %q: %w", s.ID, err)
	}
	return nil
}
...
func (r *SkillRepository) Delete(ctx context.Context, id string) error {
	if id == "" {
		return errors.New("postgres SkillRepository: empty id")
	}
	_, err := r.db.ExecContext(ctx,
		`DELETE FROM skills WHERE id = $1`, id,
	)
	if err != nil {
		return fmt.Errorf("postgres skills Delete %q: %w", id, err)
	}
	return nil
}
