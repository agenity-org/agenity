...
func (r *AuthSecretRepository) Get(ctx context.Context, purpose string) (*persistence.AuthSecret, error) {
	if purpose == "" {
		return nil, errors.New("postgres AuthSecretRepository: empty purpose")
	}
	...
	if errors.Is(err, sql.ErrNoRows) {
		return nil, fmt.Errorf("postgres auth_secrets Get %q: not found", purpose)
	}
	if err != nil {
		return nil, fmt.Errorf("postgres auth_secrets Get %q: %w", purpose, err)
	}
	return &s, nil
}
...
func (r *AuthSecretRepository) Save(ctx context.Context, purpose string, key []byte, alg string) error {
	if purpose == "" {
		return errors.New("postgres AuthSecretRepository: empty purpose")
	}
	if alg == "" {
		return errors.New("postgres AuthSecretRepository: empty algorithm")
	}
	...
	if err != nil {
		return fmt.Errorf("postgres auth_secrets Save %q: %w", purpose, err)
	}
	return nil
}
