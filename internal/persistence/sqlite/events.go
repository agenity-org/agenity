...
func (r *EventRepository) Append(ctx context.Context, e persistence.Event) error {
	if e.ID == "" {
		return errors.New("sqlite EventRepository: empty event ID")
	}
	if e.Kind == "" {
		return errors.New("sqlite EventRepository: empty event Kind")
	}
	...
	if err != nil {
		return fmt.Errorf("sqlite events Append %q: %w", e.ID, err)
	}
	return nil
}
...
func (r *EventRepository) List(ctx context.Context, opts persistence.EventListOpts) ([]persistence.Event, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite events List: %w", err)
	}
	defer rows.Close()
