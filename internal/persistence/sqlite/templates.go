...
func (r *TemplateRepository) Get(ctx context.Context, id string) (*persistence.Template, error) {
	if id == "" {
		return nil, errors.New("sqlite TemplateRepository: empty id")
	}
	...
	if errors.Is(err, sql.ErrNoRows) {
		return nil, fmt.Errorf("sqlite templates Get %q: not found", id)
	}
	if err != nil {
		return nil, fmt.Errorf("sqlite templates Get %q: %w", id, err)
	}
	return &t, nil
}
...
func (r *TemplateRepository) List(ctx context.Context) ([]*persistence.Template, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite templates List: %w", err)
	}
	defer rows.Close()
...
func (r *TemplateRepository) Save(ctx context.Context, t *persistence.Template) error {
	if t == nil {
		return errors.New("sqlite TemplateRepository: nil template")
	}
	if t.ID == "" {
		return errors.New("sqlite TemplateRepository: empty ID")
	}
	...
	_, err := r.db.ExecContext(ctx,
		...
		t.ID, t.Name, t.Description, t.Body, jsonOrEmpty(t.Metadata), t.UpdatedAt,
	)
	if err != nil {
		return fmt.Errorf("sqlite templates Save %q: %w", t.ID, err)
	}
	return nil
}
...
func (r *TemplateRepository) Delete(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite templates Delete %q: %w", id, err)
	}
	return nil
}
