// internal/persistence/sqlite/audit_events.go — #489 Wave AU2.
// SQLite-backed persistence.AuditEventRepository. Follows worker2's
// D3 RBACGrantRepository pattern: thin wrapper around *sql.DB with
// JSON-coloumn marshaling at the boundary.
//
// Per V0.9.2-ARCH §10 step 24 + EPIC #459: stores audit events
// streamed up from runners over their register WS. Queries are
// per-org partitioned at the API layer (see runtimehttp/audit_query.
// go) — repository accepts the org_id as a required filter on List.
//
// Refs #489 #488.
package sqlite
...
func (r *AuditEventRepository) Save(ctx context.Context, ev *persistence.AuditEventRecord) error {
	if ev == nil {
		return errors.New("sqlite AuditEventRepository: nil event")
	}
	if ev.ID == "" {
		return errors.New("sqlite AuditEventRepository: empty id")
	}
	if ev.OrgID == "" {
		return errors.New("sqlite AuditEventRepository: empty org_id")
	}
	...
	if err != nil {
		return fmt.Errorf("sqlite audit_events Save %q: %w", ev.ID, err)
	}
	return nil
}
...
func (r *AuditEventRepository) List(ctx context.Context, opts persistence.AuditEventListOpts) ([]*persistence.AuditEventRecord, error) {
	if opts.OrgID == "" {
		return nil, errors.New("sqlite AuditEventRepository: List requires OrgID (per-org partition guard)")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite audit_events List: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("sqlite audit_events List scan: %w", err)
		}
