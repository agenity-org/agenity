// Package sqlite — ChannelRepository implementation (#655 epic #654).
package sqlite
...
func (r *ChannelRepository) Get(ctx context.Context, id string) (*persistence.Channel, error) {
	if id == "" {
		return nil, errors.New("sqlite ChannelRepository: empty id")
	}
	...
	if errors.Is(err, sql.ErrNoRows) {
		return nil, fmt.Errorf("sqlite channels Get %q: not found", id)
	}
	if err != nil {
		return nil, fmt.Errorf("sqlite channels Get %q: %w", id, err)
	}
	return &c, nil
}
...
func (r *ChannelRepository) GetByName(ctx context.Context, name string) (*persistence.Channel, error) {
	if name == "" {
		return nil, errors.New("sqlite ChannelRepository: empty name")
	}
	var c persistence.Channel
	err := r.db.QueryRowContext(ctx,
	...
	if errors.Is(err, sql.ErrNoRows) {
		return nil, fmt.Errorf("sqlite channels GetByName %q: not found", name)
	}
	if err != nil {
		return nil, fmt.Errorf("sqlite channels GetByName %q: %w", name, err)
	}
	return &c, nil
}
...
func (r *ChannelRepository) List(ctx context.Context) ([]*persistence.Channel, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite channels List: %w", err)
	}
	defer rows.Close()
...
func (r *ChannelRepository) Save(ctx context.Context, ch *persistence.Channel) error {
	if ch == nil || ch.ID == "" || ch.Name == "" {
		return errors.New("sqlite ChannelRepository.Save: id and name required")
	}
	...
	if err != nil {
		return fmt.Errorf("sqlite channels Save: %w", err)
	}
	return nil
}
...
func (r *ChannelRepository) Members(ctx context.Context, channelID string) ([]*persistence.ChannelMember, error) {
	rows, err := r.db.QueryContext(ctx,
		...
		channelID)
	if err != nil {
		return nil, fmt.Errorf("sqlite channel_members list: %w", err)
	}
	defer rows.Close()
...
func (r *ChannelRepository) AddMember(ctx context.Context, m *persistence.ChannelMember) error {
	if m == nil || m.ChannelID == "" || m.Member == "" {
		return errors.New("sqlite ChannelRepository.AddMember: channel_id and member required")
	}
	_, err := r.db.ExecContext(ctx,
...
func (r *ChannelRepository) Messages(ctx context.Context, channelID string, limit int) ([]*persistence.ChannelMessage, error) {
	...
	rows, err := r.db.QueryContext(ctx,
		...
		channelID, limit)
	if err != nil {
		return nil, fmt.Errorf("sqlite channel_messages list: %w", err)
	}
	defer rows.Close()
...
func (r *ChannelRepository) SaveMessage(ctx context.Context, msg *persistence.ChannelMessage) error {
	if msg == nil || msg.ID == "" || msg.ChannelID == "" || msg.Author == "" {
		return errors.New("sqlite ChannelRepository.SaveMessage: id, channel_id, author required")
	}
	mentionsJSON := "[]"
	...
	if err != nil {
		return fmt.Errorf("sqlite channel_messages save: %w", err)
	}
	return nil
}
