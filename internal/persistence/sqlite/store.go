...
// connected to a SQLite file. It owns the 13 Repository instances and
// returns them via the persistence.Store interface methods.
//
// NewStore opens the database, runs migrations, and returns a ready
...
func NewStore(ctx context.Context, path string) (*Store, error) {
	db, err := Open(path)
	if err != nil {
		return nil, fmt.Errorf("sqlite.NewStore: %w", err)
	}
	if err := migrate.Run(ctx, db, "sqlite"); err != nil {
		_ = db.Close()
		return nil, fmt.Errorf("sqlite.NewStore migrate: %w", err)
	}
	return NewStoreFromDB(db), nil
}

// NewStoreFromDB wraps an already-open *sql.DB (e.g., one with
// migrations already applied — useful for testing against an in-memory
// database).
func NewStoreFromDB(db *sql.DB) *Store {
	return &Store{
		...
		auditEvents: NewAuditEventRepository(db),
...
func (s *Store) AuditEvents() persistence.AuditEventRepository { return s.auditEvents }
