...
func (r *KeychainRepository) Get(ctx context.Context, key string) (string, error) {
	if key == "" {
		return "", errors.New("sqlite KeychainRepository: empty key")
	}
	var v string
	...
	if errors.Is(err, sql.ErrNoRows) {
		return "", fmt.Errorf("sqlite keychain Get %q: not found", key)
	}
	if err != nil {
		return "", fmt.Errorf("sqlite keychain Get %q: %w", key, err)
	}
	return v, nil
}
...
func (r *KeychainRepository) Set(ctx context.Context, key, value string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite keychain Set %q: %w", key, err)
	}
	return nil
}
...
func (r *KeychainRepository) Delete(ctx context.Context, key string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite keychain Delete %q: %w", key, err)
	}
	return nil
}
