...
func (r *AccountRepository) Get(ctx context.Context, id string) (*persistence.Account, error) {
	if id == "" {
		return nil, errors.New("sqlite AccountRepository: empty id")
	}
	...
	if errors.Is(err, sql.ErrNoRows) {
		return nil, fmt.Errorf("sqlite accounts Get %q: not found", id)
	}
	if err != nil {
		return nil, fmt.Errorf("sqlite accounts Get %q: %w", id, err)
	}
	return &a, nil
}
...
func (r *AccountRepository) List(ctx context.Context) ([]*persistence.Account, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite accounts List: %w", err)
	}
	defer rows.Close()
...
func (r *AccountRepository) Save(ctx context.Context, a *persistence.Account) error {
	if a == nil {
		return errors.New("sqlite AccountRepository: nil account")
	}
	if a.ID == "" {
		return errors.New("sqlite AccountRepository: empty account ID")
	}
	if a.Class == "" {
		return errors.New("sqlite AccountRepository: empty account Class")
	}
	now := time.Now().UTC()
	...
	if err != nil {
		return fmt.Errorf("sqlite accounts Save %q: %w", a.ID, err)
	}
	return nil
}
...
func (r *AccountRepository) Delete(ctx context.Context, id string) error {
	if id == "" {
		return errors.New("sqlite AccountRepository: empty id")
	}
	_, err := r.db.ExecContext(ctx,
	...
	if err != nil {
		return fmt.Errorf("sqlite accounts Delete %q: %w", id, err)
	}
	return nil
}
