...
| copilot × GitHub | ✅ FULLY RESOLVED — two gates, both fixed: (a) **PAT scope** 2026-06-19 — operator added `Copilot Requests` (Account permission) to the fine-grained PAT; vault updated + daemon restarted; live `COPILOT_AUTH_OK` + `chepherd.list → OK` + `chepherd.alert_human → OK` via the vault-injected token (real Copilot requests, credits spent). (b) **Folder-trust modal** 2026-06-20 (`50f332d`) — copilot rendered a blocking "Confirm folder trust" modal on first launch that swallowed the injected `[chepherd-knock]` marker (modal only listens for arrows/Enter/1/2/3), so qa connected MCP but showed "Session: 0 AIC used" / never called `get_task`. Fix: inject `COPILOT_ALLOW_ALL=true` (the bundle's trust gate is `env.COPILOT_ALLOW_ALL==="true" || folder∈trustedFolders`) — A/B-verified against the real binary + `copilot_env_test.go`. **Live full round-trip CONFIRMED 2026-06-20 mixed-team walk:** qa(copilot) `get_task → OK` + `alert_human → OK` ("qa online and ready" in Talk, PASS in every clean run). See docs/AGENT-CAPABILITY-MATRIX.md + "Live mixed-team scrum walk — 2026-06-20" below. | **BLOCKED on operator: chepherd-side fully done; requires the operator to add `Copilot Requests = Read` to the fine-grained PAT — no further agent action is possible (auto-recheck cron `71a83521` flips this to ✅ when the scope lands).** token VALIDATED at GitHub (`api.github.com/user → HTTP 200` — so NOT invalid/expired; the gap is ONLY the Copilot Requests scope). live CLI auth **16× over 2026-06-17/18**, every one `Authentication failed` on the same scope gap (latest **C250** `2026-06-18T07:46:05Z` — *"ensure it has the 'Copilot Requests' permission enabled"*; earlier: 9D34/D5C2/B48A/B830/CE7C/E0A0/A658/A87E/E60C/AC6E). **Auto-recheck cron `71a83521` REMOVED 2026-06-18** after ~15 consistent confirmations of the identical gated state (further automated rechecks added no new information — anti-churn, per "don't waste tokens"). **Re-test on the operator's scope-add signal → then this flips to ✅.** **misconfiguration, NOT rate-limit** (zero 429/quota). chepherd-side complete (PAT vaulted + injected + MCP HTTP — **Read-verified 2026-06-18: `vault.json` github-pat→GITHUB_TOKEN cipher present + `server.go:1075` `collectVCSTokenEnv` injects vault creds**); blocked solely on the operator's GitHub token permission — `79ddce5`/`f0ca068`/`a0d2f7f`/`062ecfb` |
| gemini-cli × Gemini free | ✅ PROVEN — full production A2A round-trip live 2026-06-17T18:35Z (free-tier-flaky) | **✅ PRODUCTION E2E FULL ROUND-TRIP PROVEN LIVE 2026-06-17T18:35Z (resolves the prior ◑, NO deploy needed):** on the RUNNING qa-gemini (free-tier `gemini-3.5-flash`), a real operator knock produced `chepherd.get_task → OK` THEN `chepherd.send_to_session→operator → OK` — the complete A2A round-trip (knock → get_task → reply delivered to operator). **Daemon MCP tool-call log artifact confirms it (re-verified 2026-06-18): `[chepherd-mcp] qa-gemini: tools/call chepherd.get_task → OK` + `chepherd.send_to_session→operator → OK` — 2× each in the daemon's own log.** The 20/day quota reset over the day gave fresh requests, so the previously deploy-gated reply leg worked end-to-end on free tier. So: the reply leg WORKS when a quota slot is available; it's **flaky on the 20/day 3.5-flash** (fails once exhausted), and the `c9ff5d0` model-pin (2.5-flash) makes it reliable. Earlier evidence retained below. **PROVEN: emits chepherd MCP tool calls.** (a) `chepherd.get_task → OK` returned the task body (operator pane: `@qa-gemini what is 7×8?`); `chepherd.list → OK` 04:15:39Z = full turn. (b) outbound reply (`send_to_session`) dies right after get_task. **Post-reset re-test 2026-06-17T15:40 CST (cron 30eaff67): get_task ✓ then `Usage limit reached for gemini-3.5-flash` again** — the 3.5-flash native 20/day is *still* capped even post-reset. BUT a live 2.5-flash probe returned quota AVAILABLE, so the model-pin fix **`c9ff5d0`** (routes gemini-cli to gemini-2.5-flash) is the concrete unblocker for the reply leg; the running qa-gemini predates the fix (still on 3.5-flash). **`c9ff5d0` MECHANISM VERIFIED LIVE 2026-06-17T08:57Z (non-disruptive — no daemon rebuild):** applied `settings.model.name=gemini-2.5-flash` to a gemini-cli then ran a one-shot — it **completed a full turn, called `chepherd.list → OK`, and reported the result** ("active agent names are tech-lead, qa-gemini, qa-pro") with **NO quota error**. So deploying `c9ff5d0` routes gemini-cli to 2.5-flash where it completes turns + calls chepherd tools — the reply leg (`send_to_session`) is just another tool call in that completed turn. Reliable full round-trip NOW via lean-coder+gemini; gemini-cli reliable once `c9ff5d0` is deployed. **[SUPERSEDED by the 18:35Z success at the top of this cell]** Earlier state — the production E2E reply leg was UNWALKED at the time; the concern then was: the running qa-gemini booted on 3.5-flash and gemini-cli reads `settings.model` only at boot — it can't be hot-switched (a container restart breaks the daemon's knock-delivery PTY attachment). A true production walk requires `c9ff5d0` DEPLOYED (daemon rebuild+restart), which kills all 3 running agents (tech-lead/qa-gemini/qa-pro) — operator-gated; operator away, so not done unprompted. The fix MECHANISM is proven (`90bfc8a`: gemini-cli on 2.5-flash completed a turn + `chepherd.list → OK` + reported results, no quota error); production E2E is pending the deploy. **Non-deploy workaround ATTEMPTED + FAILED 2026-06-17:** drove the `/api/v1/sessions/{name}/attach` WS to type `/model gemini-2.5-flash` into a throwaway gemini-cli, but `/model` is an interactive picker — raw-byte TUI driving left the agent broken (model→"Auto", "MCP issues detected", knock unprocessed). So the TUI workaround is unreliable; the **clean production walk requires the `c9ff5d0` deploy** (rebuild+restart). **[SUPERSEDED 2026-06-17T18:35Z — the production E2E reply-leg walk SUCCEEDED live (see the ✅ headline at the top of this cell); this ◑ "deferred to deploy" verdict no longer holds.]** Earlier ◑ verdict (while the reply leg was still unwalked): gemini-cli inbound (`get_task`) verified live; reply leg committed (`c9ff5d0`) + mechanism-verified (`90bfc8a`) + test-gated (`828d9f4`); the production walk was deferred to an operator deploy **until the 18:35Z quota-reset window let it complete on the running agent with no deploy** (the non-deploy WS workaround had failed earlier, but the natural quota reset resolved it). To complete: operator runs the deploy (then a fresh gemini agent walks the round-trip ✅). For free gemini in the mesh NOW, use **lean-coder+gemini** (proven full round-trip). — `b930ef4`/`c9ff5d0`/`90bfc8a`/`828d9f4` |
| opencode × Gemini free | ✅ CAPABILITY PROVEN (full round-trips live); ⚠️ free-tier-FLAKY when ≥2 run concurrently | FAILS on Cerebras/Groq (`Tokens per minute limit exceeded`, req 15–40k > 30k/12k TPM); moved to Gemini + fixed 2 daemon issues (tools-slim bug disabling chepherd tools removed in runtime.go; Google key injected as `GOOGLE_GENERATIVE_AI_API_KEY`) → **full autonomous round-trip live** (`get_task`+`alert_human`+`send_to_session→operator` → OK), re-proven 2026-06-20 (architect+full-stack both round-tripped on flash-lite in the clean5 run). **2026-06-20 free-Gemini ceiling characterized (knock plumbing PERFECT — opencode calls `get_task` even when its model won't reply, so this is NOT a chepherd defect):** `gemini-2.5-flash` = 20 req/DAY free cap (`GenerateRequestsPerDayPerProjectPerModel-FreeTier`), exhausted by a day of testing; `gemini-2.5-flash-lite` = has daily quota but a weaker model that intermittently "exits loop" after `get_task` without composing the reply (~1-of-2 completes); `gemini-3-flash-preview` = capable but tight preview rate-limits → `stream error` under 2-agent load. So a single opencode is reliable; **two concurrent heavy opencode on ONE free Gemini key is the bottleneck** — fix is the daily reset (flash), a paid key, staggering, or lean-coder for free high-volume. See "Live mixed-team scrum walk — 2026-06-20" below. |
...
## Live mixed-team scrum walk — 2026-06-20 (operator's exact roster)

Roster: `product-owner`/claude-code, `scrum-master`/gemini-cli, `architect`/opencode,
`full-stack`/opencode, `qa`/copilot. Criterion: full round-trip (knock → `get_task` →
reply tool), verified against the **per-message Talk transcript window** (not cumulative
daemon logs, which over-count across runs).

**Two code fixes shipped + verified this session:**
- **Talk transcript** (`e060c9c`) — agent replies now attributed correctly (`agent → ['operator']`),
  no longer collapsed to "user". Confirmed live (this whole walk reads the transcript).
- **copilot folder-trust modal** (`50f332d`) — `COPILOT_ALLOW_ALL=true`; qa(copilot) now round-trips.

**Run results (same roster, different free-Gemini model for the 3 Gemini-backed slots):**

| Run | Gemini model (gemini-cli + 2× opencode) | claude (PO) | gemini-cli (scrum) | copilot (qa) | opencode (architect) | opencode (full-stack) | clean total |
|---|---|---|---|---|---|---|---|
| finish_walk | gemini-2.5-flash | ✅ | ✅ | ✅ | ❌ flash 20/day exhausted | ✅ | 4/5 |
| clean5 (flash-lite) | gemini-2.5-flash-lite | (spawn race) | (race) | (race) | ✅ | ✅ | 2 opencode ✅ |
| clean5b | gemini-2.5-flash-lite | ✅ | ✅ | ✅ | ✅ | ❌ model exited loop after get_task | 4/5 |
| clean5c | gemini-3-flash-preview | ✅ | ✅ | ✅ | ❌ exited loop | ❌ stream-error rate-limit | 3/5 |

**Verdict:**
- **claude-code, gemini-cli, copilot = 100% reliable** (passed every clean run). copilot was the
  session's headline fix and is now solid.
- **opencode = capability PROVEN, free-tier-FLAKY for concurrency.** Each opencode round-trips
  individually (clean5 both, clean5b architect); the failures are 100% free-Gemini model behavior,
  never chepherd — the daemon delivers the knock and opencode calls `get_task` even on the runs where
  the model then declines to emit the reply tool.
- **Root cause of "not a clean 5/5 snapshot": one shared free Gemini key can't serve 2 concurrent
  heavy opencode agents** — `gemini-2.5-flash` (reliable) is daily-capped at 20 req/day (exhausted by
  testing, resets midnight PT); `gemini-2.5-flash-lite` has quota but is too weak to reliably finish
  the 2-step; `gemini-3-flash-preview` is capable but preview-rate-limited. This is a **capacity/model
  ceiling on the free tier**, the exact axis distinct from binary capability.
- **To reach a deterministic simultaneous 5/5 of THIS roster:** (a) wait for the flash daily reset and
  run on `gemini-2.5-flash`; or (b) a paid Gemini key (operator-forbidden on free-only policy); or
  (c) give each opencode its own free key; or (d) swap one opencode → lean-coder (proven 100% on
  Cerebras/Groq) for a free, deterministic 5/5 with a different roster. `OPENCODE_MODEL` restored to
  the operator's original `google/gemini-2.5-flash`.
