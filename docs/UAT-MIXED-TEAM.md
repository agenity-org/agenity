...
| copilot × GitHub | ⏳ **PARKED — awaiting operator action** | **PARKED: requires the operator to add `Copilot Requests = Read` to the fine-grained PAT — no further agent action is possible.** live CLI auth **6×**, all `Authentication failed` (Req IDs 9D34/D5C2/B48A/B830/**CE7C** — latest 2026-06-17T04:47:05Z, `Copilot Requests` perm still absent); **misconfiguration, NOT rate-limit** (zero 429/quota). chepherd-side complete (PAT vaulted + injected + MCP HTTP); blocked solely on the operator's GitHub token permission — `79ddce5`/`f0ca068`/`a0d2f7f`/`062ecfb` |
| gemini-cli × Gemini free | ⚠️ partial on free (inbound ✓, outbound reply quota-bound) | **PROVEN: emits chepherd MCP tool calls.** (a) `chepherd.get_task → OK` returned the task body (operator pane: `@qa-gemini what is 7×8?`); `chepherd.list → OK` 04:15:39Z = full turn. (b) outbound reply (`send_to_session`) dies on the 20/day cap right after get_task (`Usage limit reached for gemini-3.5-flash`) — so the full round-trip is quota-bound on free; reliable via paid key or lean-coder+gemini. — `b930ef4`/`c9ff5d0` |
