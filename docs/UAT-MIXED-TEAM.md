...
| opencode × Gemini free | ✅ RESOLVED 2026-06-19 | FAILS on Cerebras/Groq (`Tokens per minute limit exceeded`, req 15–40k > 30k/12k TPM); moved to Gemini (~250k TPM) + fixed 2 daemon issues (tools-slim bug disabling chepherd tools removed in runtime.go; Google key injected as `GOOGLE_GENERATIVE_AI_API_KEY`) → **full autonomous round-trip live** (`get_task`+`alert_human`+`send_to_session→operator` → OK). See AGENT-CAPABILITY-MATRIX.md |
