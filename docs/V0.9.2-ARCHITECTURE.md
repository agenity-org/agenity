...
**Status:** Canon. This is the canonical architecture of the chepherd daemon/runtime. v0.9.2 is the latest released git tag; v0.9.4 is in active development (QA complete, not yet tagged — see CHANGELOG `[Unreleased]`). The architecture described here remains the target shape for both. Where v0.9.4-era reality has diverged or extended this design (notably the live federation mesh), the relevant sections call it out inline.

**Authority.** Authored from architect deliberation in [#205](https://github.com/agenity-org/agenity/issues/205) and prior context in [#186](https://github.com/agenity-org/agenity/issues/186). Reflects all decisions from the v0.9.2 architecture review. Living updates land via ADR + semver bump where they change a wire protocol.

**Audience.** The developer building chepherd + the chepherd.org public hub + iogrid integration. Read this before writing any code.

> **Component-line note.** The version numbers in this document (`v0.9.2`, `v0.9.4`) refer to the chepherd **daemon/runtime** (the Go `chepherd` binary). The chepherd-rc **clients** (web, iOS, Android, relay) are a separate component line versioned independently at `chepherd-rc v0.2.0-rc3`. Do not conflate the two.
...
| Retired Python "supervisor" daemon | Single Go `chepherd` binary (daemon + runner modes); the supervisor codename no longer exists |
| OpenOva codename used in conversations | **chepherd.org** is the canonical name for the managed public hub |

> **Vocabulary discipline.** Project-cadence terms — *Wave N*, *Phase X* (as a release-planning unit), *MVP* — are banned in architecture docs. Use feature names or neutral phrasing instead. (Numbered handshake/protocol steps inside a single sequence diagram are referred to as "Step N", not "Phase N".)
>
> **Federation is live, not retired.** Earlier drafts proposed dropping the word "federation". Ground reality has overtaken that: the cross-host mesh is live and is named federation. Independent parties rendezvous through a managed hub (`signal.openova.io`) and exchange A2A over hub-relayed WebRTC with zero inbound ports. STUN-assisted P2P and TURN relay are both supported. See Section 12 (discovery) and Section 19 (hub federation).
...
| 54 | Operator Account | State | Per-operator identity + bindings (OIDC subject, email, organization, role, claude-token / per-provider LLM credential references, billing customer id). Distinct from #51 LLM credential vault (which is iogrid-side platform-held credentials); #54 holds operator-side credential bindings + identity claims used for daemon RBAC + dashboard authentication. | Inside chepherd-daemon | DB inside chepherd-daemon (`accounts` table backed by AccountRepository) | Referenced by #7 RBAC grants (granter_org/grantee_org); written by operator-onboarding flows; bound at session-spawn time to attach token bindings to runners |
...
- Spec published at https://github.com/agenity-org/agenity-p2p-extension (or similar) under Apache 2.0
...
### Recipe confidentiality threat model (explicit, v0.9.2)

The doc claims recipes are "encrypted at rest in iogrid". This protects against SPECIFIC threats. Explicitly:

| Threat | Protected? | How |
|---|---|---|
| Customer reads skill author's system prompt | Yes | Customer never sees system prompt; only input + output |
| Storage breach of iogrid's recipe DB | Yes | Recipes encrypted at rest with iogrid-held key (AES-256-GCM) |
| Other skill authors read this author's recipe | Yes | Per-author access controls; recipes only decrypted in spawn context |
| iogrid operator with infrastructure access reads system prompt | NOT protected in v0.9.2 | iogrid runtime decrypts at spawn; operators with privileged access can read |
| LLM provider (e.g., Anthropic) sees the prompt | NOT protected | Inherent to using a third-party LLM; the prompt IS sent to the LLM |

**Skill authors must trust iogrid as a platform** in v0.9.2 — the same way you trust GitHub not to leak your private repos. iogrid commits to operational security (SOC2-grade controls roadmap) but cannot cryptographically guarantee operator-level confidentiality in v0.9.2.

**True confidentiality from iogrid itself** (TEE-based execution where iogrid CANNOT see prompts even with infrastructure access) is a v1.x roadmap item requiring confidential-compute hardware integration. Skill authors needing this should self-host chepherd-runtime + iogrid-equivalent or wait for TEE support. The TEE work is a separate architectural project, not a v0.9.2 deferral within the same release.
...
sequenceDiagram
    ...
    Note over A,C: Step 0, Agent Card registration at spawn one-time
    ...
    Note over A,C: Step 1, Discovery via chepherd.org directory
    ...
    Note over A,C: Step 2, Cross-org credential request
    ...
    Note over A,C: Step 3, chepherd-aware client tries P2P first
    Note over RA: see x-chepherd-p2p extension in agent-C card

    Note over A,C: Step 4, ICE candidate gathering RFC 8445
    ...
    Note over A,C: Step 5, Signaling SDP exchange
    ...
    Note over A,C: Step 6, ICE connectivity checks
    Note over RA,RC: pair candidates, STUN binding probes, nominate winning pair

    Note over A,C: Step 7, DTLS handshake on nominated pair
    ...
    Note over A,C: Step 8, A2A over WebRTC DataChannel P2P direct
    ...
    Note over A,C: Step 9, Result back via P2P DataChannel
    RC->>RA: A2A GetTask response with result
    Note right of RC: still over connection 24 direct
    RA->>A: forwarded result

    Note over A,C: Step 10, Audit local to each org
    par audit
...
sequenceDiagram
    ...
    Note over A,C: Step 1, Discovery via direct Agent Card fetch
    A->>RA: discover_peer
    RA->>CPX: query for capability X
    ...
    Note over A,C: Step 2, Credential request direct to CPY
    A->>RA: SendMessage to agent-C URL
    RA->>CPX: request JWT
    ...
    Note over A,C: Step 3-6 SIMPLIFIED, no STUN no TURN no ICE no WebRTC
    Note over RA,CPY: CPY is publicly addressable, standard TLS suffices

    Note over A,C: Step 7, A2A SendMessage direct to CPY public endpoint
...
#### Federation peer registry (cross-host mesh)

For the live cross-host mesh (Section 19), the rendezvous hub at `signal.openova.io` exposes a peer registry that independent private deployments use to find each other without either side opening inbound ports:

- `POST /v1/registry/announce` — a party publishes its presence and Agent Card pointer
- `GET /v1/registry/peers` — enumerate the current mesh

Once a peer is discovered here, A2A traffic flows over hub-relayed WebRTC (STUN-assisted P2P, TURN relay fallback).
...
#### Live federation mesh (current reality)

A live federation mesh runs against a managed rendezvous hub at `signal.openova.io` (chepherd-hub). Independent parties (each its own private chepherd deployment) discover each other through the hub's peer registry and exchange A2A over hub-relayed WebRTC, with **zero inbound ports** on either side:

- **Peer registry / discovery.** Hub exposes `POST /v1/registry/announce` (a party publishes its presence + Agent Card pointer) and `GET /v1/registry/peers` (enumerate the current mesh). This is the cross-host equivalent of the per-org curated directory in Section 12.2.
- **Hub-relayed WebRTC A2A.** Daemons/runners hold an outbound WebSocket to the hub for signaling; A2A traffic then flows over a WebRTC DataChannel. STUN-assisted P2P is attempted first; **TURN relay** through the hub is the fallback when ICE cannot establish a direct path.
- **Org-id constraint.** Org identifiers in the mesh must be dot-free (they map onto hub routing labels).
- **TURN on Kubernetes.** Running pion TURN's ephemeral relay ports under k8s requires `hostNetwork` (a plain `hostPort 3478` is insufficient and collides with existing services on some hosts). Hubs that cannot grant `hostNetwork` run STUN-assisted-P2P only until hardened.
...
5. **If a new A2A spec version is released**, audit this doc's Section 6 against the new spec. Update method names, state names, auth scheme names. This doc tracks A2A spec DRAFT v1.0 at `a2aproject/A2A@main`; re-fetch the upstream spec before stacking spec-dependent work.

6. **Wire-protocol changes go through ADR + semver.** Any change to an over-the-wire format (A2A envelope handling, the `x-chepherd-p2p` extension schema, the federation registry endpoints, the headless API) requires an ADR and a daemon semver bump. PROTOCOL.md is the normative reference for those wire formats; this doc describes how they fit together.
